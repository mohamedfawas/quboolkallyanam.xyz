...
func (h *UserHandler) GetMatchRecommendations(c *gin.Context) {
	...
	request := dto.GetMatchRecommendationsRequest{
		Limit:  int32(limit),
		Offset: int32(offset),
	}

	response, err := h.userUsecase.GetMatchRecommendations(ctx, request)
	if err != nil {
	...
	apiresponse.Success(c, "Match recommendations retrieved successfully", response)
}
