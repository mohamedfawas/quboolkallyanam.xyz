...
// @Description List profiles that the authenticated user has liked
// @Tags User
// @Produce json
...
// @Security BearerAuth
// @Router /api/v1/user/matches/liked [get]
func (h *UserHandler) GetLikedProfiles(c *gin.Context) {
	...
	req := dto.GetProfilesByMatchActionRequest{
		Action: validation.MatchMakingOptionLiked,
		Limit:  int32(limit64),
	...
	if err != nil {
		if apperrors.ShouldLogError(err) {
			log.Error("Failed to get liked profiles", zap.Error(err))
		}
		apiresponse.Error(c, err, nil)
		return
	}

	log.Info("Liked profiles retrieved successfully")
	apiresponse.Success(c, "Liked profiles retrieved successfully", resp)
}
