...
// @Summary Get target user details by profile ID (admin)
// @Description Admin can view any user's profile, partner preferences (if any), and additional photos (if any).
// @Tags Admin
// @Produce json
...
// @Security BearerAuth
// @Router /api/v1/admin/users/{profile_id}/details [get]
func (h *UserHandler) GetUserDetailsByProfileIDForAdmin(c *gin.Context) {
	...
	if err != nil {
		if apperrors.ShouldLogError(err) {
			log.Error("Failed to get user details by profile id (admin)", zap.Error(err))
		}
		apiresponse.Error(c, err, nil)
		return
	}

	log.Info("Fetched user details by profile id (admin)",
